...
class SocketService {
  ...
  connect() {
    ...
    if (this.connectAttempts >= this.maxConnectAttempts) {
      console.warn(
        `Reached maximum connection attempts (${this.maxConnectAttempts}). Stopping reconnection.`
      );
      return null;
    }
    ...
    console.log(
      `Connecting to WebSocket server at: ${SOCKET_URL} (Attempt ${this.connectAttempts}/${this.maxConnectAttempts})`
    );
    ...
    this.socket.on("connect_error", (error) => {
      ...
      if (this.connectAttempts >= this.maxConnectAttempts) {
        console.warn(
          "Maximum connection attempts reached, running in offline mode"
        );
      }
    });
  ...
  on(event, callback) {
    ...
    if (
      this.isConnected &&
      (event === "sensor-update" || event === "sensor_update")
    ) {
  ...
  reconnect() {
    ...
    if (!socket && this.connectAttempts < this.maxConnectAttempts) {
      this.reconnectInterval = setInterval(() => {
        ...
        if (this.connectAttempts >= this.maxConnectAttempts) {
          console.warn(
            "Maximum reconnection attempts reached, stopping reconnect interval"
          );
          clearInterval(this.reconnectInterval);
          this.reconnectInterval = null;
        }
